...
internal static class CatalogEndpoints
{
    ...
    {
        var items = await dbContext.Products.AsNoTracking().OrderBy(product => product.Name).ToListAsync(cancellationToken);
    ...
    private static async Task<IResult> GetProductByIdAsync(Guid id, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        return product is null ? Results.NotFound(new { message = "Product not found." }) : Results.Ok(ApiMappings.ToProductResponse(product));
    }

    private static async Task<IResult> CreateProductAsync(CreateProductRequest request, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        var product = new Domain.Entities.Product(request.Sku.Trim(), request.Name.Trim(), request.Category.Trim(), request.SupplierCode.Trim(), request.UnitCost);
        dbContext.Products.Add(product);
        await dbContext.SaveChangesAsync(cancellationToken);
        return Results.Created($"/api/products/{product.Id}", ApiMappings.ToProductResponse(product));
    }

    private static async Task<IResult> UpdateProductAsync(Guid id, UpdateProductRequest request, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        product.UpdateDetails(request.Name.Trim(), request.Category.Trim(), request.SupplierCode.Trim(), request.UnitCost);
        await dbContext.SaveChangesAsync(cancellationToken);
        return Results.Ok(ApiMappings.ToProductResponse(product));
    }

    private static async Task<IResult> ArchiveProductAsync(Guid id, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        var product = await dbContext.Products.SingleOrDefaultAsync(item => item.Id == id, cancellationToken);
        if (product is null)
        {
    ...
    private static async Task<IResult> GetWarehousesAsync(LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        var inventoryCounts = await dbContext.InventoryItems
            .AsNoTracking()
            .GroupBy(item => item.WarehouseId)
            .Select(group => new WarehouseInventoryCounts(group.Key, group.Count(), group.Count(item => item.QuantityOnHand - item.QuantityReserved <= item.ReorderThreshold)))
            .ToDictionaryAsync(item => item.WarehouseId, cancellationToken);

        var items = await dbContext.Warehouses.AsNoTracking().OrderBy(warehouse => warehouse.Name).ToListAsync(cancellationToken);
    ...
    private static async Task<IResult> GetWarehouseByIdAsync(Guid id, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> CreateWarehouseAsync(CreateWarehouseRequest request, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> UpdateWarehouseAsync(Guid id, UpdateWarehouseRequest request, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        await dbContext.SaveChangesAsync(cancellationToken);
        return await GetWarehouseByIdAsync(id, dbContext, cancellationToken);
    }

    private static async Task<IResult> DeactivateWarehouseAsync(Guid id, LayeredMonolithDbContext dbContext, CancellationToken cancellationToken)
    {
        var warehouse = await dbContext.Warehouses.SingleOrDefaultAsync(item => item.Id == id, cancellationToken);
        if (warehouse is null)
        {
